 ...
 * @since      1.0.0
 */
class TrueReview_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truereview-recent posts-thumbnail-widget',
			'description' => esc_html__( 'Display recent posts with thumbnails.', 'truereview' )
		);

		// Create the widget.
		parent::__construct(
			'truereview-recent',                                           // $this->id_base
			esc_html__( '&raquo; Recent Posts Thumbnails', 'truereview' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'truereview_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'truereview_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'truereview_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'truereview' ),
			'limit'     => 5,
			'show_date' => false,
			'style'     => 'default'
		);

 ...
 * @since      1.0.0
 */
class TrueReview_Video_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truereview-video video-widget',
			'description' => esc_html__( 'Easily to display any type of video.', 'truereview' )
		);

		// Create the widget.
		parent::__construct(
			'truereview-video',                  // $this->id_base
			esc_html__( '&raquo; Video', 'truereview' ), // $this->name
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'      => esc_html__( 'Video', 'truereview' ),
			'video_url'  => '',
			'video_code' => ''
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'video_url' ); ?>">
				<?php esc_html_e( 'Video URL', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'video_code' ); ?>">
				<?php esc_html_e( 'Embed Code:', 'truereview' ); ?>
			</label>

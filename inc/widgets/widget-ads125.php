 ...
 * @since      1.0.0
 */
class TrueReview_Ads125_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget_ads_125',
			'description' => esc_html__( 'Easily to display any type of ad.', 'truereview' )
		);

		// Create the widget.
		parent::__construct(
			'truereview-ad125',                                          // $this->id_base
			esc_html__( '&raquo; Advertisement 125x125', 'truereview' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['title'] ) {
			...
			if ( $instance['ad_code1'] ) {
				echo '<span class="ads-left">' . stripslashes( $instance['ad_code1'] ) . '</span>';
			}

			if ( $instance['ad_code2'] ) {
				echo '<span class="ads-right">' . stripslashes( $instance['ad_code2'] ) . '</span>';
			}

			if ( $instance['ad_code3'] ) {
				echo '<span class="ads-left">' . stripslashes( $instance['ad_code3'] ) . '</span>';
			}

			if ( $instance['ad_code4'] ) {
				echo '<span class="ads-right">' . stripslashes( $instance['ad_code4'] ) . '</span>';
			}
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ad_code1' ); ?>">
				<?php esc_html_e( 'Ad Code 1:', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ad_code2' ); ?>">
				<?php esc_html_e( 'Ad Code 2:', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ad_code3' ); ?>">
				<?php esc_html_e( 'Ad Code 3:', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ad_code4' ); ?>">
				<?php esc_html_e( 'Ad Code 4:', 'truereview' ); ?>
			</label>

 ...
 * @since      1.0.0
 */
class TrueReview_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truereview-popular posts-thumbnail-widget',
			'description' => esc_html__( 'Display popular posts by comments with thumbnails.', 'truereview' )
		);

		// Create the widget.
		parent::__construct(
			'truereview-popular',                                           // $this->id_base
			esc_html__( '&raquo; Popular Posts Thumbnails', 'truereview' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'truereview_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'truereview_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'truereview_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'truereview' ),
			'limit'     => 5,
			'show_date' => false,
			'style'     => 'default'
		);

 ...
 * @since      1.0.0
 */
class TrueReview_Feedburner_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truereview-feedburner widget_newsletter',
			'description' => esc_html__( 'FeedBurner email subscription.', 'truereview' )
		);

		// Create the widget.
		parent::__construct(
			'truereview-feedburner',                  // $this->id_base
			esc_html__( '&raquo; Feedburner', 'truereview' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['feed_id'] ) { ?>
			<form class="form-subscribe" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo strip_tags( $instance['feed_id'] ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520'); return true">
				...
				<?php } ?>
				<input type="text" name="email" placeholder="<?php esc_attr_e( 'Enter your email', 'truereview' ); ?>">
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'   => esc_html__( 'Get Updates', 'truereview' ),
			'before'  => esc_html__( 'Subscribe to our newsletter to receive breaking news by email.', 'truereview' ),
			'feed_id' => '',
			'button_text' => esc_html__( 'Signup', 'truereview' )
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'before' ); ?>">
				<?php esc_html_e( 'HTML or Text before form:', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'feed_id' ); ?>">
				<?php esc_html_e( 'Feedburner ID:', 'truereview' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'button_text' ); ?>">
				<?php esc_html_e( 'Button Text', 'truereview' ); ?>
			</label>
